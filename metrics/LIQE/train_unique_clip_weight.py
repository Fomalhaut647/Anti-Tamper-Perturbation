...
dist_texts = torch.cat(
    [clip.tokenize(f"a photo with {c} artifacts") for c in dists]
).to(device)
distmap_texts = torch.cat(
    [clip.tokenize(f"a photo with {c} artifacts") for c in dists_map]
).to(device)
quality_texts = torch.cat(
...
if mtl == 1:
    joint_texts = torch.cat(
        [
            clip.tokenize(f"a photo of a {c}, which is of {q} quality")
            for q, c in product(qualitys, scenes)
        ]
    ).to(device)
elif mtl == 2:
    joint_texts = torch.cat(
        [
            clip.tokenize(f"a photo with {d} artifacts, which is of {q} quality")
            for q, d in product(qualitys, dists_map)
        ]
    ).to(device)
...
def calc_loss3(
    quality_pred, quality_gt, num_sample_per_task, distortion_pred, distortion_gt
):
...
def train(model, best_result, best_epoch, srcc_dict, scene_dict, type_dict):
    ...
    for step in range(num_steps_per_epoch):
        ...
        for dataset_idx, loader in enumerate(loaders, 0):
            ...
            x, gmos, dist, scene1, scene2, scene3, valid = (
                sample_batched["I"],
                sample_batched["mos"],
                sample_batched["dist_type"],
                sample_batched["scene_content1"],
                sample_batched["scene_content2"],
                sample_batched["scene_content3"],
                sample_batched["valid"],
            )
            x = x.to(device)
            gmos = gmos.to(device)
        ...
        if mtl == 0:
            ...
            total_loss = (
                loss_m4(logits_quality, num_sample_per_task, gmos_batch.detach()).mean()
            ...
            all_loss = calc_loss(
                ...
                num_sample_per_task,
                logits_distortion,
                dist_gt_batch.detach(),
                logits_scene,
                scene_gt_batch.detach(),
            )
        elif mtl == 1:
        ...
        format_str = (
            "(E:%d, S:%d / %d) [Loss = %.4f] (%.1f samples/sec; %.3f " "sec/batch)"
        )
        print(
            format_str
            % (
    ...
    all_result = {"val": {}, "test": {}}
    if epoch >= 0:
        scene_acc1, dist_acc1, srcc1 = eval(
            live_val_loader, phase="val", dataset="live"
        )
        scene_acc11, dist_acc11, srcc11 = eval(
            live_test_loader, phase="test", dataset="live"
        )

        scene_acc2, dist_acc2, srcc2 = eval(
            csiq_val_loader, phase="val", dataset="csiq"
        )
        scene_acc22, dist_acc22, srcc22 = eval(
            csiq_test_loader, phase="test", dataset="csiq"
        )

        scene_acc3, dist_acc3, srcc3 = eval(bid_val_loader, phase="val", dataset="bid")
        scene_acc33, dist_acc33, srcc33 = eval(
            bid_test_loader, phase="test", dataset="bid"
        )

        scene_acc4, dist_acc4, srcc4 = eval(
            clive_val_loader, phase="val", dataset="clive"
        )
        scene_acc44, dist_acc44, srcc44 = eval(
            clive_test_loader, phase="test", dataset="clive"
        )

        scene_acc5, dist_acc5, srcc5 = eval(
            koniq10k_val_loader, phase="val", dataset="koniq10k"
        )
        scene_acc55, dist_acc55, srcc55 = eval(
            koniq10k_test_loader, phase="test", dataset="koniq10k"
        )

        scene_acc6, dist_acc6, srcc6 = eval(
            kadid10k_val_loader, phase="val", dataset="kadid10k"
        )
        scene_acc66, dist_acc66, srcc66 = eval(
            kadid10k_test_loader, phase="test", dataset="kadid10k"
        )
        ...
        all_result["val"] = {
            "quality": quality_result["val"],
            "scene": scene_result["val"],
            "distortion": distortion_result["val"],
        }
        all_result["test"] = {
            "quality": quality_result["test"],
            "scene": scene_result["test"],
        ...
        scene_avg = (
            scene_acc1 + scene_acc2 + scene_acc3 + scene_acc4 + scene_acc5 + scene_acc6
        ) / 6

        dist_avg = (
            dist_acc1 + dist_acc2 + dist_acc3 + dist_acc4 + dist_acc5 + dist_acc6
        ) / 6
        ...
        if current_avg > best_result["avg"]:
            ...
            ckpt_name = os.path.join(
                "checkpoints", str(session + 1), "avg_best_ckpt.pt"
            )
            torch.save(
                {
                    "epoch": epoch,
        ...
        if srcc_avg > best_result["quality"]:
            ...
            ckpt_name = os.path.join(
                "checkpoints", str(session + 1), "quality_best_ckpt.pt"
            )
            torch.save(
                {
                    "epoch": epoch,
        ...
        if scene_avg > best_result["scene"]:
            ...
            ckpt_name = os.path.join(
                "checkpoints", str(session + 1), "scene_best_ckpt.pt"
            )
            torch.save(
                {
                    "epoch": epoch,
        ...
        if dist_avg > best_result["distortion"]:
            ...
            ckpt_name = os.path.join(
                "checkpoints", str(session + 1), "distortion_best_ckpt.pt"
            )
            torch.save(
                {
                    "epoch": epoch,
    ...
    for step, sample_batched in enumerate(loader, 0):
        ...
        if mtl != 1:
            ...
            for i in range(len(dist)):
                if (
                    dist_map[dist[i]] == dists_map[indice2[i]]
                ):  # dist_map: mapping dict #dists_map: type list
                    correct_dist += 1
                num_dist += 1
        if mtl != 2:
            for i in range(len(valid)):
                if valid[i] == 1:
                    ...
                    if (scene1[i] == scenes[indices[i, 0]]) | (
                        scene1[i] == scenes[indices[i, 1]]
                    ):
                        correct_scene += 1
                    if (scene2[i] == scenes[indices[i, 0]]) | (
                        scene2[i] == scenes[indices[i, 1]]
                    ):
                        correct_scene += 1
                    num_scene += 2
                elif valid[i] == 3:
                    ...
                    indices = indices.squeeze()
                    if (
                        (scene1[i] == scenes[indices[i, 0]])
                        | (scene1[i] == scenes[indices[i, 1]])
                        | (scene1[i] == scenes[indices[i, 2]])
                    ):
                        correct_scene += 1
                    if (
                        (scene2[i] == scenes[indices[i, 0]])
                        | (scene2[i] == scenes[indices[i, 1]])
                        | (scene2[i] == scenes[indices[i, 2]])
                    ):
                        correct_scene += 1
                    if (
                        (scene3[i] == scenes[indices[i, 0]])
                        | (scene3[i] == scenes[indices[i, 1]])
...
for session in range(0, 10):
    ...
    live_train_csv = os.path.join(
        "../IQA_Database/databaserelease2/splits2",
        str(session + 1),
        "live_train_clip.txt",
    )
    live_val_csv = os.path.join(
        "../IQA_Database/databaserelease2/splits2",
        str(session + 1),
        "live_val_clip.txt",
    )
    live_test_csv = os.path.join(
        "../IQA_Database/databaserelease2/splits2",
        str(session + 1),
        "live_test_clip.txt",
    )

    csiq_train_csv = os.path.join(
        "../IQA_Database/CSIQ/splits2", str(session + 1), "csiq_train_clip.txt"
    )
    csiq_val_csv = os.path.join(
        "../IQA_Database/CSIQ/splits2", str(session + 1), "csiq_val_clip.txt"
    )
    csiq_test_csv = os.path.join(
        "../IQA_Database/CSIQ/splits2", str(session + 1), "csiq_test_clip.txt"
    )

    bid_train_csv = os.path.join(
        "../IQA_Database/BID/splits2", str(session + 1), "bid_train_clip.txt"
    )
    bid_val_csv = os.path.join(
        "../IQA_Database/BID/splits2", str(session + 1), "bid_val_clip.txt"
    )
    bid_test_csv = os.path.join(
        "../IQA_Database/BID/splits2", str(session + 1), "bid_test_clip.txt"
    )

    clive_train_csv = os.path.join(
        "../IQA_Database/ChallengeDB_release/splits2",
        str(session + 1),
        "clive_train_clip.txt",
    )
    clive_val_csv = os.path.join(
        "../IQA_Database/ChallengeDB_release/splits2",
        str(session + 1),
        "clive_val_clip.txt",
    )
    clive_test_csv = os.path.join(
        "../IQA_Database/ChallengeDB_release/splits2",
        str(session + 1),
        "clive_test_clip.txt",
    )

    koniq10k_train_csv = os.path.join(
        "../IQA_Database/koniq-10k/splits2", str(session + 1), "koniq10k_train_clip.txt"
    )
    koniq10k_val_csv = os.path.join(
        "../IQA_Database/koniq-10k/splits2", str(session + 1), "koniq10k_val_clip.txt"
    )
    koniq10k_test_csv = os.path.join(
        "../IQA_Database/koniq-10k/splits2", str(session + 1), "koniq10k_test_clip.txt"
    )

    kadid10k_train_csv = os.path.join(
        "../IQA_Database/kadid10k/splits2", str(session + 1), "kadid10k_train_clip.txt"
    )
    kadid10k_val_csv = os.path.join(
        "../IQA_Database/kadid10k/splits2", str(session + 1), "kadid10k_val_clip.txt"
    )
    kadid10k_test_csv = os.path.join(
        "../IQA_Database/kadid10k/splits2", str(session + 1), "kadid10k_test_clip.txt"
    )

    live_train_loader = set_dataset(
        live_train_csv, 4, live_set, num_workers, preprocess3, train_patch, False
    )
    live_val_loader = set_dataset(
        live_val_csv, 16, live_set, num_workers, preprocess2, 15, True
    )
    live_test_loader = set_dataset(
        live_test_csv, 16, live_set, num_workers, preprocess2, 15, True
    )

    csiq_train_loader = set_dataset(
        csiq_train_csv, 4, csiq_set, num_workers, preprocess3, train_patch, False
    )
    csiq_val_loader = set_dataset(
        csiq_val_csv, 16, csiq_set, num_workers, preprocess2, 15, True
    )
    csiq_test_loader = set_dataset(
        csiq_test_csv, 16, csiq_set, num_workers, preprocess2, 15, True
    )

    bid_train_loader = set_dataset(
        bid_train_csv, 4, bid_set, num_workers, preprocess3, train_patch, False
    )
    bid_val_loader = set_dataset(
        bid_val_csv, 16, bid_set, num_workers, preprocess2, 15, True
    )
    bid_test_loader = set_dataset(
        bid_test_csv, 16, bid_set, num_workers, preprocess2, 15, True
    )

    clive_train_loader = set_dataset(
        clive_train_csv, 4, clive_set, num_workers, preprocess3, train_patch, False
    )
    clive_val_loader = set_dataset(
        clive_val_csv, 16, clive_set, num_workers, preprocess2, 15, True
    )
    clive_test_loader = set_dataset(
        clive_test_csv, 16, clive_set, num_workers, preprocess2, 15, True
    )

    koniq10k_train_loader = set_dataset(
        koniq10k_train_csv,
        16,
        koniq10k_set,
        num_workers,
    ...
    koniq10k_val_loader = set_dataset(
        koniq10k_val_csv, 16, koniq10k_set, num_workers, preprocess2, 15, True
    )
    koniq10k_test_loader = set_dataset(
        koniq10k_test_csv, 16, koniq10k_set, num_workers, preprocess2, 15, True
    )

    kadid10k_train_loader = set_dataset(
        kadid10k_train_csv,
        16,
        kadid10k_set,
        num_workers,
    ...
    kadid10k_val_loader = set_dataset(
        kadid10k_val_csv, 16, kadid10k_set, num_workers, preprocess2, 15, True
    )
    kadid10k_test_loader = set_dataset(
        kadid10k_test_csv, 16, kadid10k_set, num_workers, preprocess2, 15, True
    )
    ...
    train_loaders = [
        ...
        bid_train_loader,
        clive_train_loader,
        koniq10k_train_loader,
        kadid10k_train_loader,
    ]

    result_pkl = {}
    for epoch in range(0, num_epoch):
        best_result, best_epoch, srcc_dict, scene_dict, type_dict, all_result = train(
            model, best_result, best_epoch, srcc_dict, scene_dict, type_dict
        )
        scheduler.step()
