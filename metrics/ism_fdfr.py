...
def compute_face_embedding(img_path):
    ...
    try:
        ...
        if resps == 1:
            ...
            resps = list(resps)
            resps.sort(
                key=lambda resp: resp["facial_area"]["h"] * resp["facial_area"]["w"],
                reverse=True,
            )
            return np.array(resps[0]["embedding"])
    except Exception:
...
def matching_score_genimage_id(images_path, list_id_path):
    ...
    if fail_detection_count != len(image_list):
        return (
            ave_ism / (len(image_list) - fail_detection_count),
            fail_detection_count / len(image_list),
            avg_embedding,
        )
...
def parse_args():
    ...
    parser.add_argument(
        "--data_dir", type=str, default="", required=True, help="path to datadir"
    )
    parser.add_argument(
        "--emb_dirs",
        metavar="N",
        type=str,
        nargs="+",
        help="list of paths to clean image",
    )
