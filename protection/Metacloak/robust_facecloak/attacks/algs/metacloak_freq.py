...
def main(args):
    ...
    gau_filter = transforms.GaussianBlur(
    ...
    all_trans = (
        train_aug + tensorize_and_normalize
    )  # remove EOT sampling for fast convergence. ATP don't need this for improving robustness.
    ...
    pbar = tqdm(
        total=args.total_trail_num
        * num_models
        * (args.interval // args.advance_steps)
        * len(steps_list),
        desc="meta poison with model ensemble",
    )
    cnt = 0
        ...
        for model_i in range(num_models):
            ...
            for split_step in steps_list:
                ...
                text_encoder.load_state_dict(
                    init_model_state_pool[model_i][split_step]["text_encoder"]
                )
                f = [unet, text_encoder]

                for j in range(args.interval // args.advance_steps):

                    perturbed_data = defender.perturb_freq(
                        f,
                        perturbed_data,
                        original_data,
                        vae,
