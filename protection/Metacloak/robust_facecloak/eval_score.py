...
def loop_to_get_overall_score(
    gen_image_dir, clean_ref_dir="", func_get_score_of_one_image=None, type_name="face"
):
...
class ScoreEval:
    def __init__(
        self,
        func_get_score_of_one_image=lambda image_dir, clean_ref_dir, type_name="face": 0,
    ):
        self.func_get_score_of_one_image = func_get_score_of_one_image

    def __loop_to_get_overall_score__(
        self, gen_image_dir, clean_ref_db=None, type_name="face"
    ):
        files_db_gen = glob.glob(os.path.join(gen_image_dir, "*.png"))
        files_db_gen += glob.glob(os.path.join(gen_image_dir, "*.jpg"))
        scores = []
        ...
        for i in range(len(files_db_gen)):
            gen_i = files_db_gen[i]
            score = self.func_get_score_of_one_image(
                gen_i, clean_ref_db, type_name=type_name
            )
            scores.append(score)
        # filter out nan and np.inf
        scores = np.array(scores)
    ...
    def __call__(self, gen_image_dir, clean_ref_db=None, type_name="face"):
        return self.__loop_to_get_overall_score__(
            gen_image_dir, clean_ref_db, type_name=type_name
        )
...
LIQE_Scene_Human_Scorer = ScoreEval(
    func_get_score_of_one_image=LIQE_get_scene_human_score
)
...
def IMS_CLIP_get_score(gen_i, clean_ref_db, type_name="face"):
    ...
    if os.path.exists(ref_pkl_path):
        ...
        ref_representation_mean = (
            ref_representation_mean / ref_representation_mean.norm(dim=-1, keepdim=True)
        )
    ...
    cosine_similarity = (
        (ref_representation_mean.cpu().numpy() * image_features.cpu().numpy())
        .sum()
        .mean()
    )
...
def CLIP_IQAC_get_score(gen_i, clean_ref_db=None, type_name="face"):
    ...
    image = clip_preprocess(gen_img).unsqueeze(0).to("cuda")
    text = clip.tokenize(
        ["a good photo of " + type_name, "a bad photo of " + type_name]
    ).to("cuda")
    similarity_matrix = None
    with torch.no_grad():
        image_features = clip_model.encode_image(image)
...
def CLIP_zero_short_classification_get_score(
    gen_i, clean_ref_db=None, type_name="face"
):
    import torch
    ...
    image = clip_preprocess(gen_img).unsqueeze(0).to("cuda")
    text = clip.tokenize(
        ["a picture of " + type_name, "a picture of non-" + type_name]
    ).to("cuda")
    similarity_matrix = None
    with torch.no_grad():
        image_features = clip_model.encode_image(image)
...
CLIP_zero_short_classification_Scorer = ScoreEval(
    func_get_score_of_one_image=CLIP_zero_short_classification_get_score
)
...
def IMS_get_score(
    ...
    dfs = find_without_savepkl(
        img_path=gen_i,
        db_path=clean_ref_db,
...
def get_score(
    ...
    result_dict = {}
    if type_name == "face":
        result_dict["SDS"] = FDSR_Scorer(
            gen_image_dir=image_dir, clean_ref_db=clean_ref_dir, type_name=type_name
        )
        result_dict["CLIP_Face_IQA"] = CLIP_Face_Scorer(
            gen_image_dir=image_dir, clean_ref_db=clean_ref_dir, type_name=type_name
        )
        result_dict["LIQE_Scene_Human"] = LIQE_Scene_Human_Scorer(
            gen_image_dir=image_dir, clean_ref_db=clean_ref_dir, type_name=type_name
        )
    else:
        CLIP_zero_short_classification_score = CLIP_zero_short_classification_Scorer(
            gen_image_dir=image_dir, clean_ref_db=clean_ref_dir, type_name=type_name
        )
        result_dict["SDS"] = CLIP_zero_short_classification_score

    result_dict["CLIPIQA"] = CLIP_IQA_Scorer(
        gen_image_dir=image_dir, clean_ref_db=clean_ref_dir, type_name=type_name
    )
    result_dict["BRISQUE"] = BRISQUE_Scorer(
        gen_image_dir=image_dir, clean_ref_db=clean_ref_dir, type_name=type_name
    )
    result_dict["LIQE_Quality"] = LIQE_Quality_Scorer(
        gen_image_dir=image_dir, clean_ref_db=clean_ref_dir, type_name=type_name
    )
    result_dict["IMS_CLIP_ViT-B/32"] = IMS_CLIP_Scorer(
        gen_image_dir=image_dir, clean_ref_db=clean_ref_dir, type_name=type_name
    )
    result_dict["CLIP_IQAC"] = CLIP_IQAC_Scorer(
        gen_image_dir=image_dir, clean_ref_db=clean_ref_dir, type_name=type_name
    )

    if type_name == "face":
        result_dict[f"IMS_VGG-Face_cosine"] = IMS_Face_Scorer(
            gen_image_dir=image_dir, clean_ref_db=clean_ref_dir, type_name=type_name
        )
