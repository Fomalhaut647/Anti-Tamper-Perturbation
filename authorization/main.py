...
model = HIDNet(
    device,
    gamma=gamma,
    randomFlag=random_maskFlag,
    input_mask=input_mask,
    pixel_space=pixel_space,
)
val_dataloader = torch.utils.data.DataLoader(
    TestDataset(path=config["data"]["val_data_path"]), batch_size=10, shuffle=False
)
train_dataloader = torch.utils.data.DataLoader(
    FFHQDataset(path=config["data"]["train_data_path"]),
...
this_run_folder = utils.create_folder_for_run(
    run_folder, config["train"]["store_folder"]
)
...
for _ in range(epoch_num):
    ...
    for batch_id, image in tqdm(enumerate(train_dataloader)):
        ...
        message = utils.get_phis(
            phi_dimension=message_length, batch_size=image.shape[0]
        ).to(device)
        losses, (encoded_images, random_mask, decoded_messages) = model.train_one_batch(
            [image, message]
        )
        ...
        if global_step % val_interval == 0:
            ...
            for batch_id, image in enumerate(val_dataloader):
                ...
                losses, (
                    ...
                    random_mask,
                    decoded_messages,
                ) = model.val_one_batch([image, message])
                for key in losses:
        ...
        if global_step % save_interval == 0:
            utils.save_checkpoint(
                model,
                experiment_name=config["train"]["store_folder"],
                epoch=global_step,
